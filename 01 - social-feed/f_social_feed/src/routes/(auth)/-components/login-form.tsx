  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Afficher le message dans le toast
        toast.error(result.message || 'Erreur de connexion')

        // Si les erreurs contiennent le même message que result.message,
        // c'est une erreur générale d'authentification (pas d'affichage sous les champs)
        if (result.errors && result.errors.email) {
          const errorMessage = result.errors.email[0]
          if (errorMessage !== result.message) {
            // C'est une erreur de validation du champ email
            form.setError('email', {
              type: 'manual',
        ...
        // Gérer les autres erreurs de champs
        if (result.errors) {
